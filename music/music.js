...
var HelloWorldLayer = cc.Layer.extend({
    ...
    self.music=[[1,1,5,5,6,6,5,4,4,3,3,2,2,1,5,5,4,4,3,3,2,5,5,4,4,3,3,2],[1,2,3,5,6,3,2,1,4],[2,3,4,5,3,4,5,6],[5,6,7,3,1,2,3,4],[1,2,3,4,6,5,4]];
    //self.music=[1,2,3,4,5];
    self.now=0;//当前执行的music音节数组中的序号
    self.add_animate(2);//装好星星真动画,idel眨眼
    //将发射音符的定时器附在这个没用的精灵上
    self.one=new cc.Sprite(res.bg);
    self.addChild(self.one,-1);
    /////////////////////////////////////////////////////////
    //场景布置
    /////////////////////////////////////////////////////////
    self.create(res.bg,1,1,self.size.width/2,self.size.height/1.4,3,1,1);//总背景
    self.create(res.bg_under,1,1,self.size.width/2,self.size.height/2,1.1,1,1);//琴的背景
    //7个按键+7个隐藏键
    for(var i=0;i<7;i++){
        self.create(res.key_all,1,1,self.size.width/14*(2*(i+1)-1),self.size.height/2.95,2,1,1);
    }
    for(var i=0;i<7;i++){
        self.create(res["key"+(i+1)],1,1,self.size.width/14*(2*(i+1)-1),self.size.height/2.95,2,1,1);
    }
    //7个彩色按键隐藏属性
    for(var i=9;i<16;i++) {
    ...
    //星星16-20
    self.create(res.start1,1,1,self.size.width/5,self.size.height/1.1,3,1,1);
    self.create(res.start1,1,1,self.size.width/14*13,self.size.height/1.22,3,1,1);
    self.create(res.start1,1.4,1.4,self.size.width/2.6,self.size.height/1.36,3,1,1);
    self.create(res.start1,0.6,0.6,self.size.width/14*9,self.size.height/1.3,3,1,1);
    self.create(res.start1,0.9,0.9,self.size.width/14,self.size.height/1.4,3,1,1);
    //星星动画16-20
    ...
    //倒计时数字 21-23
    self.create(res.one,1.3,1.5,self.size.width/2,self.size.height/2,10,1,1);
    self.sprite_array[21].setOpacity(0);
    self.create(res.two,1,1.5,self.size.width/2,self.size.height/2,10,1,1);
    self.create(res.three,1,1.5,self.size.width/2,self.size.height/2,10,1,1);
    self.sprite_array[22].setOpacity(0);
    self.sprite_array[23].setOpacity(0);
    //倒计时数字下的背景
    self.create(res.bg_mask,1.3,1.3,self.size.width/2,self.size.height/2,9,1,1);
    ...
    click_sound:function(location,sprite,num){
        if(cc.rectContainsPoint(sprite.getBoundingBox(),location)){
            cc.audioEngine.playEffect(res["m"+num], false);
    ...
    add_animate:function(animate_num){
        ...
        for(var i=1;i<(animate_num+1);i++){
            self.anima.addSpriteFrameWithFile(res["start"+i]);
        }
        self.anima.addSpriteFrameWithFile(res["start"+1]);
    },
    ...
    click_star: function (location) {
        for(var i=16;i<21;i++){
            if(cc.rectContainsPoint(self.sprite_array[i].getBoundingBox(),location)){
                ...
                self.sprite_array[i].setVisible(false);
                //23
                self.create(res.start3,self.sprite_array[i].scale,self.sprite_array[i].scale,self.sprite_array[i].getPosition().x,self.sprite_array[i].getPosition().y,10,1,1);
            ...
            self.sprite_array[22].runAction(cc.sequence(cc.fadeIn(0),cc.scaleBy(0.5,2),cc.scaleBy(0.5,0.5),cc.fadeOut(0),cc.callFunc(function(){
                self.sprite_array[21].runAction(cc.sequence(cc.fadeIn(0),cc.scaleBy(0.5,2),cc.scaleBy(0.5,0.5),cc.fadeOut(0),cc.callFunc(function(){
                    ...
                    self.one.schedule(function(){
                        self.create(res["point"+self.music[i][self.now]],0.3,0.3,self.size.width/14*(2*self.music[i][self.now]-1),self.size.height/1.8,4,self.music[i][self.now],2);
    ...
    point_boom:function(location,num){
        var boom=new cc.Sprite(res["boom"+num]);
        boom.scale=0.5;
